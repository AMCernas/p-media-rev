...
export function DraftsClient({
  ...
  const [drafts, setDrafts] = useState<EnrichedReview[]>(initialDrafts);
  ...
  useEffect(() => {
    ...
    setTotal(totalCount);
  }, [initialDrafts, initialHasMore, totalCount]);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDrafts(prev => [...prev, ...data.reviews]);
        ...
        <>
          ...
          {!hasMore && drafts.length > 0 && (
            <div className="mt-8 text-center text-sm text-[#52525b]">
              Mostrando {drafts.length} de {total} borradores
            </div>
          )}
        </>
      ) : (
